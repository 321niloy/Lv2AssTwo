...
import { ProductValidationSchema, searchQuerySchema } from "./product.validation";
...
const createProduct = async(req:Request, res:Response) =>{
   try{
    ...
    const result = await Productservice.createProductintoDb(prouctfromValidation);
   ...
   res.status(500).json({
    success:false,
    messeage:error.message || "Oops!! Product unSuccessfully Created",
    data:error
   })
   }
}
...
const getProducts = async (req:Request , res:Response) =>{
    try{
     ...
     if(searchTerm && typeof searchTerm === 'string'){
        ...
        const result = await Productservice.searchFromProductDb(persedQuery.searchTerm);
        ...
        res.status(500).json({
            success:false,
            messege:error.message || "Oops!! Product unSuccessfully Created",
            data:error
        })
    }
}
...
const updateSingelProduct = async (req:Request , res: Response) =>{
    try{
        ...
        const result = await Productservice.updateSingelProductDb(productId , updatedata)
        res.status(200).json({
        success:true,
